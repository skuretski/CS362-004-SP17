...
int main() {
	...
	testG.hand[p1][0] = adventurer;
	testG.hand[p1][1] = embargo;
	testG.hand[p1][2] = village;
	testG.hand[p1][3] = cutpurse;
	testG.hand[p1][4] = tribute;
	testG.deck[p1][0] = copper;
	testG.deck[p1][1] = minion;
	testG.deck[p1][2] = smithy;
	testG.deck[p1][3] = silver;
	testG.deck[p1][4] = gold;

	cardEffect(adventurer, choice1, choice2, choice3, &testG, handpos, &bonus);

	newCards = 4; // total drawn for this specific card set
	discarded = 2; // total non-treasure cards discarded for this specific card set
	treasures = 2; // total treasure cards drawn for this specific card set
	xtraCoins = 3; // total coin amount drawn for this specific card set (1 copper, 1 silver)
#if (NOISY_TEST == 1)
