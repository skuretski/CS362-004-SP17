...
int main(int argc, char **argv){
	...
	int randNum, result, discardBefore, discardAfter, error = 0;	
	randNum = rand()%123;
	...
	if(result != 0){
		printf("Error:\tinitializing game returned non-zero value.\n\n");
		error = 1;
	}
	...
	if(result != 0){
		printf("Error:\tplaying Adventurer card returned non-zero value.\n\n");
		error = 1;
	}
	...
	if(game.handCount[0] != 6){
		printf("Error:\tAdventurer card did not put at least 1 card into player's hand.\n");
		printf("\t* Hand count should be 6 (+2 treasure cards and discard Adventurer.)\n\n");
		error = 1;
	}
	else if(game.handCount[0] <= 5){
		printf("Error:\tAdventurer card did not put any cards into player's hand.\n\n");
		error = 1;
	}
	...
	if(cardDrawn1 != copper || cardDrawn2 != copper){
		printf("Error:\tThe last drawn cards should be copper.\n\n");
		error = 1;
	}
	...
	if(discardAfter < 1){
		printf("Error:\tNo cards were discarded after playing Adventurer.\n\n");
		error = 1;
	}
	if(error == 0)
